...
<h1>REMOVED_FACEBOOK_APP_ID1 - an interactive mobile application for parenting</h1>
...
After each time pod install, copy [CP-User] [RNFB] Core Configuration && [CP-User] [RNFB] Crashlytics Configuration in each target’s build phases if missing from main target (ParentBuddyApp). You can use the below targets.
  ...
  **Project Target:**
   1. ParentBuddyApp - REMOVED_FACEBOOK_APP_ID1 Production target
   2. ParentBuddyAppDev - REMOVED_FACEBOOK_APP_ID1 Development target
   3. ParentBuddyAppXk - REMOVED_FACEBOOK_APP_ID1 Production target
   4. ParentBuddyAppXkDev - REMOVED_FACEBOOK_APP_ID1 Development target
...
8. Steps required while creating Flavour builds for both REMOVED_FACEBOOK_APP_ID1 and REMOVED_FACEBOOK_APP_ID1. 

   **1.** Follow below steps for **REMOVED_FACEBOOK_APP_ID1 Prod and ProdStaging**:
   
      * In apiConstants.ts change buildFor const value to buildForREMOVED_FACEBOOK_APP_ID1 as follows:
         
         ```
         export const buildFor = buildForREMOVED_FACEBOOK_APP_ID1;
         ```
...
- Enter the app name as "REMOVED_FACEBOOK_APP_ID1" and follow the instructions to complete the setup.
...
- Name your project (e.g., "REMOVED_FACEBOOK_APP_ID1 Production") and click "Create."
