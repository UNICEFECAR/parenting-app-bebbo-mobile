...
const EditParentDetails = ({ route, navigation }: Props): any => {
  const { userParentalRoleData, userRelationToParentEdit, parentEditName } =
    route.params;
  const [relationship, setRelationship] = useState(
    userParentalRoleData ? userParentalRoleData : ""
  );
  ...
  const [parentName, setParentName] = React.useState(
    parentEditName ? parentEditName : ""
  );
  const headerColor = themeContext?.colors.PRIMARY_COLOR;
  const relationshipToParent =
    relationshipToParentGlobal.length > 0 && userParentalRoleData != ""
      ? relationshipToParentGlobal.find(
          (o: any) => o.id === userParentalRoleData
        )
      : "";
  const relationshipValue = relationshipData.find(
    (item: any) => item.id === userRelationToParentEdit
  );
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      setUserRelationToParent(
        relationshipToParent != "" &&
          relationshipToParent != null &&
          relationshipToParent != undefined
          ? relationshipToParent.unique_name
          : ""
      );
  ...
  const saveParentData = async (
    relationship: any,
    parentName: any,
    userRelationToParent: any
  ): Promise<any> => {
    const relationshipnew: any = relationship;
    if (
      typeof relationshipnew === "string" ||
      relationshipnew instanceof String
    ) {
      ...
      <View style={[styles.flex1, { backgroundColor: bgcolorWhite }]}>
        ...
        <MainManageProfileContainer>
          ...
          <View>
            ...
            userRelationToParent != appConfig.relationShipFatherId ? (
              <FormContainer1>
                        ...
                        if (
                          ...
                          if (
                            typeof taxonomyIds?.femaleData.unique_name ===
                              "string" ||
                            taxonomyIds?.femaleData.unique_name instanceof
                              String
                          ) {
                            setRelationship(
                              taxonomyIds?.femaleData.unique_name
                            );
                          } else {
                            setRelationship(
                          ...
                          if (
                            item.id == taxonomyIds?.relationShipMotherId ||
                            item.id == taxonomyIds?.relationShipFatherId
                          ) {
                            setRelationship("");
                          }
                        }
