...
const AddChildHealthCheckup = ({ route, navigation }: any): any => {
  ...
  const [editHCDate, seteditHCDate] = useState<any>(
    editMeasurementDate ? editMeasurementDate : null
  );
  ...
  const deleteHealthCheckup = async (): Promise<any> => {
    if (editHCDate) {
      const existingMeasure = getMeasuresForDate(
        DateTime.fromJSDate(new Date(editHCDate)),
        activeChild
      );
      //delete measure obj
      const deleteresult =
  ...
  useEffect(() => {
    if (editMeasurementDate) {
      ...
      setDefaultMeasured(
        existingMeasure?.isChildMeasured == true
          ? isMeasuredOptions[0]
          : isMeasuredOptions[1]
      );
      ...
      const existingMeasuredVaccines = checkIfMeasuredVaccineExistsForLocale(
        existingMeasure?.vaccineIds &&
          existingMeasure?.vaccineIds != "" &&
          existingMeasure?.vaccineIds != null
          ? checkIfMeasuredVaccineExistsForLocale(
              JSON.parse(existingMeasure?.vaccineIds)
            )
          : []
      );
      if (existingMeasuredVaccines?.length > 0) {
        existingMeasuredVaccines?.forEach((element: any) => {
          element["id"] = allVaccinePeriods.find(
            (item: any) => item.uuid == element?.uuid
          )?.id;
          element["uuid"] = element?.uuid;
          element["title"] = allVaccinePeriods.find(
            (item: any) => item.uuid == element?.uuid
          )?.title;
          element["isMeasured"] = true;
          element["pinned_article"] = allVaccinePeriods.find(
            (item: any) => item.uuid == element?.uuid
          )?.pinned_article;
        });
        setTakenVaccine(existingMeasuredVaccines);
        setTakenVaccineForPrevPeriod(existingMeasuredVaccines);
        setIsVaccineMeasured(
          existingMeasuredVaccines?.length > 0 ? true : false
        );
        setDefaultVaccineMeasured(
          existingMeasuredVaccines?.length > 0
            ? isMeasuredOptions[0]
            : isMeasuredOptions[1]
        );
      } else {
        setIsVaccineMeasured(
          existingMeasuredVaccines?.didChildGetVaccines ? true : false
        );
        setDefaultVaccineMeasured(
          existingMeasuredVaccines?.didChildGetVaccines
            ? isMeasuredOptions[0]
            : isMeasuredOptions[1]
        );
      }
  ...
  const onmeasureDateChange = (event: any, selectedDate: any): any => {
    ...
    if (selectedDate) {
      ...
      if (editMeasurementDate) {
        ...
        if (
          isGrowthMeasureExistForDate(
            DateTime.fromJSDate(new Date(selectedDate)),
            activeChild
          ) ||
          isVaccineMeasureExistForDate(
            DateTime.fromJSDate(new Date(selectedDate)),
            ...
            [
              {
                ...
                onPress: (): any => {
                  ...
                  const existingMeasuredVaccines =
                    existingMeasure?.vaccineIds &&
                    existingMeasure?.vaccineIds != "" &&
                    existingMeasure?.vaccineIds != null
                      ? checkIfMeasuredVaccineExistsForLocale(
  ...
  const isFormDisabled = (): any => {
    if (measureDate) {
      const modifiedTakenVaccines = takenVaccine
        .filter((item) => item["isMeasured"] == true)
        .map(({ uuid }) => ({ uuid }));
      const allVaccines: any = [
        ...plannedVaccine,
        ...prevPlannedVaccine,
        ...modifiedTakenVaccines,
      ];
        ...
        const growthValues = {
          ...
          didChildGetVaccines: isVaccineMeasured
            ? allVaccines.length > 0
              ...
              : false
            : false,
          vaccineIds: isVaccineMeasured
            ? allVaccines.length > 0
              ? JSON.stringify(allVaccines)
              : ""
            : "",
          ...
          if (createresult?.length > 0) {
            ...
            if (isVaccineMeasured) {
              const eventData = {
                ...
                params: {
                  age_id: activeChild?.taxonomyData?.id,
                  vaccine_id: allVaccines,
                },
              };
              logEvent(eventData, netInfo.isConnected);
            }
            const eventData = {
              name: HEALTH_CHECKUP_ENTERED,
              params: { age_id: activeChild?.taxonomyData?.id },
            };
            logEvent(eventData, netInfo.isConnected);
          }
          setClicked(false);
