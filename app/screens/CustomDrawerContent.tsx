...
import { bebboShareMailId, buildFor, buildForREMOVED_FACEBOOK_APP_ID1, folejaShareMailId, shareText } from '@assets/translations/appOfflineData/apiConstants';
...
const CustomDrawerContent = ({ navigation }: any): any => {
  ...
  const onShare = async (): Promise<any> => {
    const localeData = (String(buildFor) != buildForREMOVED_FACEBOOK_APP_ID1) ? languageCode : "";
              ...
              <DrawerLinkView
                onPress={(): any => {
                  if (Platform.OS === 'android') {
                    if (String(buildFor) == buildForREMOVED_FACEBOOK_APP_ID1) {
