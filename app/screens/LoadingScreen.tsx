...
const LoadingScreen = ({ route, navigation }: Props): any => {
  ...
  const getAgeWithAgeBrackets = async (prevPage: any): Promise<any> => {
    ...
    if (allDataDownloadFlag == true && prevPage != "CountryLangChange") {
      ...
      childList.map((child: any) => {
        const childAgedays = DateTime.now()
          .diff(DateTime.fromISO(child.birthDate), "days")
          .toObject().days;
        if (childAgedays) {
          if (
            childAgedays >=
            child.taxonomyData.days_to - child.taxonomyData.buffers_days
          ) {
            const i = childAge.findIndex(
              (item: any) => item?.id === child?.taxonomyData?.id
            );
  ...
  const callSagaApi = async (enableImageDownload: any): Promise<any> => {
    ...
    if (prevPage == "ChildSetup" || prevPage == "AddEditChild") {
      ...
      if (prevRoute && prevRoute.name && prevRoute.name == "DetailsScreen") {
        ...
        if (
          prevPage == "CountryLangChange" ||
          (prevPage == "DownloadAllData" && allDataDownloadFlag == false)
        ) {
          const schemaarray = [
            ArticleEntitySchema,
      ...
      // }
      const apiJsonDataarticleall = apiJsonDataGet("all");
      const apiJsonDataarticledelta = apiJsonDataGet(
        "all",
        true,
        incrementalSyncDT
      );
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log(
        incrementalSyncDT,
        "incrementalSyncDT in useeffect laoding---",
        netflag
      );
  ...
  useEffect(() => {
    if (
      ...
      dispatch(
        onLocalizationSelect({
          languages: allCountries[0]?.languages,
          countryId: allCountries[0]?.CountryID,
        })
      );
    }
  }, [isFirst]);
