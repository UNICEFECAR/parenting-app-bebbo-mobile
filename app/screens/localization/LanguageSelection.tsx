...
// import { allApisObject, appConfig, buildFor, buildForREMOVED_FACEBOOK_APP_ID1, buildForREMOVED_FACEBOOK_APP_ID1 } from '@assets/translations/appOfflineData/apiConstants';
...
const LanguageSelection = ({ route, navigation }: Props): any => {
  ...
  if (appConfig.buildFor == appConfig.buildForREMOVED_FACEBOOK_APP_ID1 && (route.params == null || route.params == undefined || route.params?.country == null)) {
    console.log("in if--");
  ...
  const goToConfirmationScreen = (): any => {
    ...
    i18n.changeLanguage(newLanguage?.locale || 'en')
      .then(() => {
        if (appConfig.buildFor == appConfig.buildForREMOVED_FACEBOOK_APP_ID1) {
