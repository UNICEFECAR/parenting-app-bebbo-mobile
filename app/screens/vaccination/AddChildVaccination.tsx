...
import { vaccinationColor } from "../../instances/bebbo/styles/style";
import {
  Heading2,
  Heading3,
  Heading3Center,
...
const AddChildVaccination = ({ route, navigation }: any): any => {
  ...
  const [editVCDate, seteditVCDate] = useState<DateTime>(
    editVaccineDate ? editVaccineDate : null
  );
  ...
  const deleteVaccination = async (): Promise<any> => {
    if (editVCDate) {
      const existingMeasure = getMeasuresForDate(
        DateTime.fromJSDate(new Date(editVCDate)),
        activeChild
      );
      //delete measure obj
      const deleteresult =
  ...
  useEffect(() => {
    if (editVaccineDate) {
      setShowDelete(true);
      const existingMeasure = getMeasuresForDate(
        DateTime.fromJSDate(new Date(editVaccineDate)),
        activeChild
      );
      ...
      if (editVCDate) {
        ...
        if (
          ...
          const growthValues = {
            ...
            didChildGetVaccines: allVaccines.length > 0 ? true : false,
            vaccineIds:
              allVaccines.length > 0 ? JSON.stringify(allVaccines) : "",
            doctorComment: remarkTxt,
            measurementPlace: 0,
          };
          const createresult =
        ...
        <HeaderRowView style={styles.headerRowStyle}>
          ...
          <HeaderTitleView>
            <Heading2 numberOfLines={1}>
              {showDelete ? t("editVcTitle") : t("addVcTitle")}
            </Heading2>
          </HeaderTitleView>
          {showDelete ? (
                      ...
                      <FDirRow>
                        <RadioOuter>
                          ...
                          >
                            <FlexFDirRowSpace>
                              <Heading3
                                style={styles.headingFlexShrink}
                                numberOfLines={2}
                              >
                                {weightValue
