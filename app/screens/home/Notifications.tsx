...
const Notifications = (): any => {
      ...
      const combinedNotis = currentChildallnoti
        ...
        .filter((item: any) => {
          return (
            ...
            toDay >=
              DateTime.fromJSDate(new Date(item.notificationDate)).toMillis() &&
            childBirthDate <=
              DateTime.fromJSDate(new Date(item.notificationDate)).toMillis()
          );
        });
    ...
    if (appState != nextAppState) {
      if (
        ...
        if (allChildnotification.length > 0) {
          const currentChildNotis = allChildnotification.find(
  ...
  const onCategorychange = async (
    selectedCategoriesParam: any
  ): Promise<any> => {
    const selectedFilters = selectedCategoriesParam
      .filter((category: any) => category.isActivated == true)
      .map((item: any) => {
        return item.type;
      });
  ...
  const onNotiItemChecked = (item: any, isChecked: boolean): any => {
    if (isChecked == true) {
      const allCheckedNotis = [...checkedNotifications, item];
  ...
  const onItemReadMarked = async (notiItem: any): Promise<any> => {
    ...
    const currentChildNotis = {
      ...allNotifications.find((item) => item.childuuid == activeChild.uuid),
    };
    const currentChildIndex = allNotifications.findIndex(
      (item) => item.childuuid == activeChild.uuid
    );
    if (notiItem.type == "gw" || notiItem.type == "cd") {
      const notitoUpdateIndex = currentChildNotis.gwcdnotis.findIndex(
        (item: any) =>
          item.days_from == notiItem.days_from &&
          item.days_to == notiItem.days_to &&
          item.type == notiItem.type
      );
      ...
      const notitoUpdateIndex = currentChildNotis.vcnotis.findIndex(
        (item: any) =>
          item.days_from == notiItem.days_from &&
          item.days_to == notiItem.days_to &&
          item.type == notiItem.type
      ...
      const notitoUpdateIndex = currentChildNotis.hcnotis.findIndex(
        (item: any) =>
          item.days_from == notiItem.days_from &&
          item.days_to == notiItem.days_to &&
          item.type == notiItem.type
      ...
      if (currentChildNotis.reminderNotis) {
        const notitoUpdateIndex = currentChildNotis.reminderNotis.findIndex(
          (item: any) =>
            item.uuid == notiItem.uuid &&
            item.type == notiItem.type &&
            item.subtype == notiItem.subtype &&
            item.subtypeid == notiItem.subtypeid
        );
        const newItem: any = { ...notiItem };
        newItem.isRead = newItem.isRead == true ? false : true;
        delete newItem.isChecked;
            ...
            <View style={styles.flex7}>
              ...
              <View style={styles.notiCategoriesView}>
                {notifications?.length > 0 ? (
                  ...
                  (selectedCategories.length > 0 &&
                    notifications.filter((item: { type: any }) =>
              ...
              <ButtonContainerTwo style={styles.buttonContainerTwo}>
                ...
                <ButtonColTwo>
                  <ButtonSecondary
                    ...
                    disabled={checkedNotifications.length == 0 ? true : false}
                  >
                    <ButtonText numberOfLines={2}>
                      {t("notiDelSelected", {
