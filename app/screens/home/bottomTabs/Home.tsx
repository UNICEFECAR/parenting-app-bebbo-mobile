...
const Home = ({ route, navigation }: any): any => {
  ...
  const onNoForceUpdate = (): any => {
    ...
    else if (netInfo.isConnected && showDownloadPopup && errorObj.length > 0) {
      setTimeout(() => {
    ...
    async function fetchNetInfo(): Promise<any> {
      ...
      if (userIsOnboarded == false) {
        ...
        if (netInfo.isConnected && showDownloadPopup) {
          if (isVideoArticleUpdateReq == null || isVideoArticleUpdateReq == undefined || isVideoArticleUpdateReq == 'true') {
            ...
            if (VersionInfo.appVersion > bebbodev && VersionInfo.bundleIdentifier == 'com.datamatics.bebbo') {
              dispatch(fetchAPI(apiJsonDatavideoart, 'VideoArticle', dispatch, navigation, languageCode, activeChild, apiJsonDatavideoart, netInfo.isConnected));
          ...
          if (isRelatedVideoArticleUpdateReq == null || isRelatedVideoArticleUpdateReq == undefined || isRelatedVideoArticleUpdateReq == 'true') {
            ...
            if (VersionInfo.appVersion > relbebbodev && VersionInfo.bundleIdentifier == 'com.datamatics.bebbo') {
              console.log("in if");
              dispatch(fetchAPI(apiJsonDatarelatedvideoart, 'RelatedVideoArticle', dispatch, navigation, languageCode, activeChild, apiJsonDatarelatedvideoart, netInfo.isConnected));
        ...
        if (netInfo.isConnected && showDownloadPopup) {
          ...
          dispatch(fetchAPI(apiJsonDatasurvey, 'Survey', dispatch, navigation, languageCode, activeChild, apiJsonDatasurvey, netInfo.isConnected));
            ...
            {show2 && (
             ...
             <HomeSearchBox>
              ...
              <SearchInput
                ...
                placeholder={t('searchInREMOVED_FACEBOOK_APP_ID1Text')}
