...
} from "@components/shared/SupportChatStyle";
  ...
  return (
    <FlexRow>
      ...
      <BotBubbleContainer>
        ...
        steps &&
        steps.textToShow &&
        steps.textToShow.answer_part_1 &&
        steps.textToShow.answer_part_1 != "" ? (
          <>
            ...
            {steps &&
            steps.textToShow &&
            steps.textToShow.answer_part_2 &&
            steps.textToShow.answer_part_2 != "" &&
            answer2visible == false ? (
              <SideSpacing15>
                ...
                >
                  <ButtonTextMdLineL>{t("readMoreTxt")}</ButtonTextMdLineL>
                </ButtonLinkPressLeft>
              </SideSpacing15>
            ) : null}
            {answer2visible == true ? (
            ...
            {steps &&
            steps.textToShow &&
            steps.textToShow.related_article &&
            steps.textToShow.related_article != 0 &&
            (answer2visible == true ||
              (steps.textToShow.answer_part_2 == "" &&
                answer2visible == false)) ? (
              <SideSpacing15>
                ...
                >
                  <ButtonTextMdLineL>{t("learnMoreLinkTxt")}</ButtonTextMdLineL>
                </ButtonLinkPressLeft>
              </SideSpacing15>
            ) : null}
          </>
  ...
  return (
    <>
      <OptionBubbleContainer>
        ...
        >
          <Heading4Centerr>{optionval?.label}</Heading4Centerr>
        </OptionBubblePressable>
      </OptionBubbleContainer>
    </>
  );
...
const ActionBubble = (props: any): any => {
  ...
  return (
    <>
      <ActionBubbleContainer
        style={actionindex == 0 ? styles.marginTop40 : styles.marginTop0}
      >
        <ActionBubblePressable
          onPress={(): any => {
            actionval?.nextStepFunc(
              stepindex,
              actionindex,
              actionval.nextStepval,
              steps[stepindex].id,
              steps,
              stepsjson
            );
...
const ChatBot: React.FC<ChatBotData> = (props: any) => {
  ...
  return (
    <View
      style={[
        styles.flex1,
        props.index == 0 ? styles.paddingTop10 : styles.paddingTop0,
      ]}
      key={props.index}
    >
      {props.item.showNextStep == true ? (
        <>
          <BotBubble
            key={"b" + props.item.id + "-" + props.index}
          ...
          {loading == false ? (
            <>
              {props.item.answer ? (
                <UserBubble
                  key={"u" + props.item.id + "-" + props.item.answer.value}
                  message={props.item.answer.label}
                  steps={props.item}
                />
              ) : (
                <>
                  {props.item.options && props.item.options.length > 0
                    ? props.item.options.map((y: any, i2: any) => {
                        return (
                          <OptionBubble
                            key={"o" + props.index + "-" + i2}
                            optionval={y}
                            optionindex={i2}
                            stepindex={props.index}
                            steps={props.steps}
                            stepsjson={props.stepsjson}
                            categorySelection={props.categorySelection}
                            dynamicStepSelection={props.dynamicStepSelection}
                            backToHomeScreen={props.backToHomeScreen}
                            showFeedbackLink={props.showFeedbackLink}
                            noDataStep={props.noDataStep}
                          />
                        );
                      })
                    : null}
                  ...
                  props.item.actions.length > 0
                    ? props.item.actions.map((y: any, i2: any) => {
                        return (
                          <ActionBubble
                            key={"a" + props.index + "-" + i2}
                            actionval={y}
                            actionindex={i2}
                            stepindex={props.index}
                            steps={props.steps}
                            stepsjson={props.stepsjson}
                            backToStep={props.backToStep}
                            backToHomeScreen={props.backToHomeScreen}
                          />
                        );
                      })
