import getAllDataToStore, {
  getAllDataOnRetryToStore,
} from "@assets/translations/appOfflineData/getDataToStore";
...
const commonApiService: CommonApiInterface = async (
  apiEndpoint: string,
  methodname: any,
  postdata: object
) => {
...
export const updateIncrementalSyncDT = async (
  response: any,
  dispatch: any,
  _navigation: any,
  ...
  const articleresp = response.find(
    (y: any) => y.apiEndpoint == appConfig.apiConfig.articles
  );
  const videoarticleresp = response.find(
    (y: any) => y.apiEndpoint == appConfig.apiConfig.videoArticles
  );
  const activitiesresp = response.find(
    (y: any) => y.apiEndpoint == appConfig.apiConfig.activities
  );
  // const faqpinnedresp = response.find((y:any)=>y.apiEndpoint == appConfig.faqPinnedContent);
  const faqsresp = response.find(
    (y: any) => y.apiEndpoint == appConfig.apiConfig.faqs
  );
  // const faqupdatedpinnedresp = response.find((y:any)=>y.apiEndpoint == appConfig.faqUpdatedPinnedContent);
  const archiveresp = response.find(
    (y: any) => y.apiEndpoint == appConfig.apiConfig.archive
  );
  if (articleresp && articleresp != null && articleresp.data) {
    if (prevPage != "AddEditChild") {
      dispatch(
        setIncrementalSyncDT({
          key: "articlesDatetime",
          value: articleresp.data.datetime,
        })
      );
    }
  }
  if (videoarticleresp && videoarticleresp != null && videoarticleresp.data) {
    dispatch(
      setIncrementalSyncDT({
        key: "videoArticlesDatetime",
        value: videoarticleresp.data.datetime,
      })
    );
  }
  if (activitiesresp && activitiesresp != null && activitiesresp.data) {
    dispatch(
      setIncrementalSyncDT({
        key: "activitiesDatetime",
        value: activitiesresp.data.datetime,
      })
    );
  }
  ...
  if (archiveresp && archiveresp != null && archiveresp.data) {
    dispatch(
      setIncrementalSyncDT({
        key: "archiveDatetime",
        value: archiveresp.data.datetime,
      })
    );
  }
};
export const onAddEditChildSuccess = async (
  response: any,
  dispatch: any,
  navigation: any,
  languageCode: string,
  ...
  const artresp = response.find(
    (x: any) => x.apiEndpoint == "articles" && x.status == 200
  );
  ...
  if (artresp && artresp != null) {
    ...
    if (artobj && artobj != null) {
      const storedata = store.getState();
      const bufferAgeBracket =
        storedata.childData.childDataSet.bufferAgeBracket;
      const childagearray =
        storedata.utilsData.taxonomy.allTaxonomyData != ""
          ? JSON.parse(storedata.utilsData.taxonomy.allTaxonomyData).child_age
          : [];
      const agesarr =
        artobj.postdata.childAge == "all"
          ? childagearray.map((x: any) => x.id)
          : artobj.postdata.childAge.split(",").map(Number);
...
export const onSponsorApiSuccess = async (
  response: any,
  dispatch: any,
  navigation: any,
  languageCode: string,
  ...
  const allDatatoStore = await getAllDataToStore(
    languageCode,
    dispatch,
    prevPage
  );
...
export const onCountryApiSuccess = async (
  response: any,
  dispatch: any,
  navigation: any,
  languageCode: string,
  ...
  if (
    response &&
    response[0] &&
    response[0].apiEndpoint == appConfig.apiConfig.countryGroups
  ) {
    response = response[0];
...
export const onOnLoadApiSuccess = async (
  _response: any,
  dispatch: any,
  navigation: any,
  languageCode: string,
  ...
  console.log("allDatatoStore ", prevPage, "--", allDatatoStore);
  const allJsonData = await userRealmCommon.getData<ChildEntity>(
    ChildEntitySchema
  );
...
export const onChildSetupApiSuccess = async (
  response: any,
  dispatch: any,
  navigation: any,
  languageCode: string,
...
export const downloadArticleImages = async (): Promise<any> => {
  ...
  const databaseData = await dataRealmCommon.getData<ArticleEntity>(
    ArticleEntitySchema
  );
  const databaseDataact = await dataRealmCommon.getData<ActivitiesEntity>(
    ActivitiesEntitySchema
  );
  const databaseDatabasicpg = await dataRealmCommon.getData<BasicPagesEntity>(
    BasicPagesSchema
  );
  const databaseDatavideoart =
    await dataRealmCommon.getData<VideoArticleEntity>(VideoArticleEntitySchema);
  ...
  databaseData.map((x: any) => {
    ...
    if (
      x["cover_image"] != "" &&
      x["cover_image"] != null &&
      x["cover_image"] != undefined &&
      x["cover_image"].url != "" &&
      x["cover_image"].url != null &&
      x["cover_image"].url != undefined &&
      (x["cover_image"].url.split("https://")[1] ||
        x["cover_image"].url.split("http://")[1])
    ) {
...
export const onHomeapiSuccess = async (
  response: any,
  dispatch: any,
  navigation: any,
  languageCode: string,
  ...
  oldErrorObj: any,
  forceupdatetime: any,
  downloadWeeklyData: any,
  downloadMonthlyData: any,
  enableImageDownload: any
): Promise<any> => {
  ...
  if (
    ...
    if (artresp && artresp != {}) {
      ...
      if (artobj && artobj != {}) {
        try {
          const storedata = store.getState();
          const childagearray = storedata.utilsData?.taxonomy?.allTaxonomyData
            ? JSON.parse(storedata.utilsData.taxonomy.allTaxonomyData)
                .child_age || []
            : [];
          const artarray =
            artobj.postdata.childAge == "all"
  ...
  if (prevPage == "DownloadUpdate" && errorObj?.length == 0) {
    const childAge =
      store.getState().utilsData.taxonomy.allTaxonomyData != ""
    ...
    if (enableImageDownload) {
      ...
      Alert.alert(
        i18n.t("downloadAllSuccessPopupTitle"),
        i18n.t("downloadAllSuccessPopupText"),
        [
          {
            text: i18n.t("downloadAllSuccessOkBtn"),
            onPress: async (): Promise<any> => {
              dispatch(
                setInfoModalOpened({ key: "allDataDownloadFlag", value: true })
              );
              navigation.reset({
                index: 0,
                routes: [
...
export const onHomeVideoartapiSuccess = async (
  _response: any,
  dispatch: any,
  _navigation: any,
  languageCode: string,
...
export const retryAlert = (): any => {
  return new Promise((resolve, reject) => {
    Alert.alert(i18n.t("retryPopupTitle"), i18n.t("retryPopupText"), [
      ...
      {
        text: i18n.t("retryRetryBtn"),
        onPress: (): any => resolve("Retry success"),
      },
    ]);
  });
};
export const cancelRetryAlert = (): any => {
  return new Promise((resolve) => {
    Alert.alert(i18n.t("cancelRetryPopupTitle"), i18n.t("cancelPopupText"), [
      {
        text: i18n.t("cancelPopUpBtn"),
        onPress: (): any => resolve("cancelRetry success"),
      },
    ]);
  });
};
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      Alert.alert(bandwidth, toggle, [
        ...
        {
          text: i18n.t("vcIsMeasuredOption1"),
          onPress: (): any => resolve("yes"),
        },
      ]);
    }, 2500);
  });
...
export const deleteArticleNotPinned = async (): Promise<any> => {
  const createresult = await dataRealmCommon.delete(
    ArticleEntitySchema.name,
    "isarticle_pinned!='1'"
  );
  return createresult;
};
