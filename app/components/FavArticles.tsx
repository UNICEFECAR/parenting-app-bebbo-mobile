...
const ContainerView = styled.View`
  ...
  background-color: ${(props: any): string =>
    props.theme.colors.ARTICLES_TINTCOLOR};
`;
      ...
      async function fetchData(): Promise<any> {
        if (favoriteadvices.length > 0) {
          const filterQuery = favoriteadvices
            .map((x: any) => `id = '${x}'`)
            .join(" OR ");
          const favData1 = await dataRealmCommon.getFilteredData<ArticleEntity>(
            ArticleEntitySchema,
            filterQuery
          );
          const favData2 =
          ...
          if (favData.length == 0) {
            favData = articleDataall.filter(
              (x: any) => favoriteadvices.findIndex((y: any) => y == x.id) > -1
            );
          }
          ...
          {favAdvicesToShow.length > 0 ? (
            <FlatList
              ...
              renderItem={({ item, index }: any): any => (
                <RenderArticleItem item={item} index={index} />
              )}
