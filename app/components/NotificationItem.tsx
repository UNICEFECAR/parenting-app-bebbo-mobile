...
const NotificationItem = (props: any): any => {
  ...
  const notiDate = DateTime.fromJSDate(
    new Date(item.notificationDate)
  ).toMillis();
  const renderGrowthNotifcation = (): any => {
    return (
      //
      toDay >= notiDate && childBirthDate <= notiDate ? (
        item.isDeleted ? null : (
          ...
          <>
            <NotificationListContainer>
              <FlexDirRowStart>
                ...
                >
                  <Icon name={geticonname(item.type)} size={20} color="#000" />
                </NotifIcon>
                <NotifiContent>
                  {item.isRead == true ? (
                    <Heading4Regular>
                ...
                <NotifAction>
                  {isDeleteEnabled === true ? (
                    ...
                    <>
                      ...
                      >
                        <MenuTrigger>
                          <Icon name={"ic_kebabmenu"} size={25} color="#000" />
                        </MenuTrigger>
                        <MenuOptions
                          customStyles={{
                            optionsContainer: {
                        ...
                        >
                          <MenuOption
                            value={1}
                            onSelect={(): any => markAsDelete(item)}
                          >
  ...
  const renderHCNotifcation = (): any => {
    return toDay >= notiDate && childBirthDate <= notiDate ? (
      item.isDeleted ? null : (
        <>
          <NotificationListContainer>
            <FlexDirRowStart>
  ...
  const renderVCNotifcation = (): any => {
    return toDay >= notiDate && childBirthDate <= notiDate ? (
      item.isDeleted ? null : getVaccinesForPeriod(item.growth_period) ==
        "" ? null : (
        <>
          <NotificationListContainer>
            <FlexDirRowStart>
              ...
              <NotifiContent>
                {item.isRead == true ? (
                  <Heading4Regular>
                    ...
                    {getVaccinesForPeriod(item.growth_period)}
                  </Heading4Regular>
                ) : (
                  <Heading4Bold>
                    {t(item.title, {
          ...
          <NotificationListContainer>
            <FlexDirRowStart>
              <NotifIcon
                style={{
                  backgroundColor:
                    item.type == "vcr" ? vaccinationColor : hcheaderColor,
                }}
              >
                <Icon name={geticonname(item.type)} size={20} color="#000" />
              </NotifIcon>
  ...
  return item.type == "gw"
    ...
    : item.type == "hc"
    ? renderHCNotifcation()
    : item.type == "vcr" || item.type == "hcr"
    ? renderVCReminderNotifcation()
    : null;
};
